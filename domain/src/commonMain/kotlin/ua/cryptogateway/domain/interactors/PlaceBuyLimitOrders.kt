...
class PlaceBuyLimitOrders(
    ...
    override suspend fun doWork(params: Params): Unit = withContext(dispatcher) {
        ...
        configs.forEach { config ->
            ...
            val quotAsset = balanceDao.getCurrency(currency = config.quoteAsset)
                .onFailure { Log.warn(it, TAG) { "Can't get balance from table for Currency = ${config.quoteAsset}" } }
            ...
            val activeOrders = orderDao.get(side = Order.Side.Buy)
                .onFailure { Log.warn(it, TAG) { "Can't get active orders side=Buy from table" } }
                .getOrNull() ?: return@forEach
            val buyLimit = config.fond - balance.balance
            if (buyLimit > 0) {    // need to buy
            if (activeOrders.isEmpty()) {
                Log.info(tag = TAG) { "Active Orders table is Empty!" }
                ...
                when {
                    activeOrdersAmount < config.orderAmount -> {
                        ...
                        Log.info(tag = TAG) { "Canceled all orders. Active amount < Config amount." }
                    ...
                    activeOrdersAmount > config.orderAmount -> {
                        ...
                        Log.info(tag = TAG) { "Canceled only a few orders: $cancelList" }
                        api.cancelOrders(cancelList)
                        // return
                    }
                    ...
                    activeOrders.minOf { it.price } != bestPrice -> {
                        val cancelList = activeOrders.map { it.id }
                        Log.info(tag = TAG) { "Canceled all orders. Min Price != Best Price." }
                        api.cancelOrders(cancelList)
                        createNewGridOrders(pairName, config, bestPrice, buyLimit, quotAssetLimit)
                        // return
                    }
                    ...
                    activeOrders.minOf { it.quantity } != startQuantity -> {
                        val cancelList = activeOrders.map { it.id }
                        Log.info(tag = TAG) { "Canceled all orders. Min Quantity != Start Quantity." }
                        api.cancelOrders(cancelList)
                        createNewGridOrders(pairName, config, bestPrice, buyLimit, quotAssetLimit)
                        // return
                    }
    ...
    private suspend fun bestPrice(pairName: String, config: BotConfigEntity): Double? {
        ...
        Log.info(tag = TAG) { "Best=$bestPrice; Ticker=$tickerPrice; Book=$bookPrice; Start=$startPrice; Bias=$biasPrice; Force=$priceForce" }
        return bestPrice
    }
    ...
    private suspend fun createNewGridOrders(
        ...
        val requests = (0 until config.orderAmount).map { item ->
            ...
            if (sumQuantity >= buyLimit || balanceLimit < 0) {
                if (balanceLimit < 0)  Log.info(tag = TAG) { "Order N$item not create! Balance limit exceeded." }
                else  Log.info(tag = TAG) { "Order N$item not create! Buy limit  exceeded." }
//                Log.info(tag = TAG) {
...
//        }
    ...
    companion object {
        private const val TAG = "PlaceBuyLimitOrders"
    }
}