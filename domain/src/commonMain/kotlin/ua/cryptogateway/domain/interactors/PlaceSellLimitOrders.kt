...
import ua.cryptogateway.domain.interactors.PlaceBuyLimitOrders.Companion
            ...
            if (activeOrders.isEmpty()) {
                ...
                when {
                    balance.balance < config.fond -> {
                        ...
                        Log.info(tag = TAG) { "Canceled all orders. Balance < Fond." }
...
//        }
    ...
    private suspend fun bestPrice(pairName: String, config: BotConfigEntity): Double? {
        ...
        val book = api.getOrderBook(pairName, 5)
            .onFailure {
                Log.warn(tag = TAG) { "Can't get public orders book from Kuna API for Pair = $pairName" }
            }
            .getOrNull()
        ...
        Log.info(tag = TAG) { "Best=$bestPrice; Ticker=$tickerPrice; Book=$bookPrice; Star=$startPrice; Bias=$biasPrice; Force=$priceForce" }
        return bestPrice
    }
    ...
    private suspend fun createNewGridOrders(
        ...
        val requests = (0 until config.orderAmount).map { item ->
            ...
            if (totalVolume >= maxVolume) {
                ...
                Log.info(tag = TAG) { "Order not create! Max Volume=$formatMaxVolume" }
//                Log.info(tag = TAG) {
